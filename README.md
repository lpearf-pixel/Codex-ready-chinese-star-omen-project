...
## Sprint 6：真实事件生成层（最小可运行链路）

一句话链路：`Ephemeris -> CelestialEvent -> OmenRule -> evidence`

- Provider：`src/astronomy/providers/skyfield_provider.py`（优先本地 skyfield，失败回退近似 stub）
- Matcher：`src/astronomy/asterism_matcher_impl.py`（心宿/角宿/房宿锚星近似）
- Detector：`src/astronomy/event_detector.py`（guarding/invading/conjunction/gathering）

Demo 命令：

```bash
python -m src.cli detect-and-match \
